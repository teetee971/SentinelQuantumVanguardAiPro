...
class NetworkSnapshotDiffTest {
    ...
    @Test fun oversizedRiskMapFailsClosed(){val risks=(0..NetworkSnapshotDiffEngine.MAX_RISK_SCORES).associate{index->index.toString(16).padStart(64,'0') to 50};val r=NetworkSnapshotDiffEngine.compare(state(1_000L,risks=risks),state(2_000L,risks=risks))!!;assertEquals(risks.size*2,r.rejectedItems);assertTrue(r.riskIncreases.isEmpty());assertTrue(r.riskDecreases.isEmpty())}
}
