...
object NetworkSnapshotDiffEngine {
    const val MAX_DEVICES=4_096; const val MAX_SERVICES=20_000; const val MAX_TOPOLOGY_EDGES=10_000; const val MAX_RISK_SCORES=4_096; const val DEFAULT_MATERIAL_RISK_DELTA=20; private const val FP_LEN=64
    ...
    private fun normalize(s:NetworkSnapshotState):Normalized {
        ...
        if(s.riskScores.size>MAX_RISK_SCORES){
            rejected+=s.riskScores.size
        }else{
            s.riskScores.toSortedMap().forEach{(raw,score)->val fp=fp(raw);if(fp==null||score !in 0..100)rejected++ else risks[fp]=score}
        }
