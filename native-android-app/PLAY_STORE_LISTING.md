...
Permissions utilisées : INTERNET, ACCESS_NETWORK_STATE, notifications optionnelles, permissions Wi-Fi/Bluetooth nécessaires aux scans locaux, accès optionnel au répertoire pour le Caller ID, rôle système CallScreeningService et permissions SMS restreintes lorsque l’utilisateur choisit Sentinel comme gestionnaire SMS par défaut. Les permissions SMS ne sont pas demandées avant `ROLE_SMS` et cessent d’être utilisées si le rôle est perdu. Aucun accès au journal d’appels ni au microphone.
