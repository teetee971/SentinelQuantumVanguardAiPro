 ...
 * CI trust requires an exact execution binding, not merely a matching commit.
 */
...
const PROVENANCE_FIELDS = Object.freeze([
  'repository', 'commit', 'workflow', 'workflow_ref',
  'run_id', 'run_attempt', 'ref', 'event',
]);
...
function reportProvenance(report) {
  if (!report?.provenance || typeof report.provenance !== 'object' || Array.isArray(report.provenance)) return null;
  return report.provenance;
}

export function validateProvenance(report, env = process.env) {
  const provenance = reportProvenance(report);
  if (!provenance) return 'CI_RUN_BINDING_INCOMPLETE';
...
function runCli() {
  ...
  if (!existsSync(input)) {
    const result = { schema_version: 2, verification_level: 'UNVERIFIED', status: 'NO_EVIDENCE', evidence_hash: null, reason: 'NO_ENGINEERING_REPORT' };
  ...
  try {
    ...
    const result = { schema_version: 2, verification_level: 'UNVERIFIED', status: 'INVALID', evidence_hash: null, reason: 'MALFORMED_ENGINEERING_REPORT' };
    writeFileSync(output, `${JSON.stringify(result, null, 2)}\n`, 'utf8');
    console.log(JSON.stringify(result, null, 2));
    process.exitCode = 2;
    return;
  ...
  const result = {
    ...
    workflow_ref: provenance?.workflow_ref ?? null,
    workflow_run_id: provenance?.run_id ?? null,
