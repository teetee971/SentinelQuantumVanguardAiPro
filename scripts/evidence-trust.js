...
export function validateProvenance(report, env = process.env) {
  ...
  if (provenance.repository !== report.repository) return 'CI_PROVENANCE_TOP_LEVEL_MISMATCH:repository';
  if (provenance.commit !== report.commit) return 'CI_PROVENANCE_TOP_LEVEL_MISMATCH:commit';
