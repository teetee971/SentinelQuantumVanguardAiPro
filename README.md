...
Le filtrage d’appels utilise `CallScreeningService`. Sa décision est renvoyée avant toute journalisation et avant l’ouverture de la fiche Caller ID. Cette fiche affiche les faits locaux disponibles : numéro normalisé, pays/drapeau, type indicatif, vérification réseau et motif Sentinel. L’utilisateur peut autoriser séparément la lecture locale du répertoire pour afficher le nom et la société du contact ; cette permission est révocable et aucun contact n’est envoyé par ce composant. L’historique Room reste limité à 500 décisions et ne conserve ni numéro brut ni numéro masqué, uniquement une empreinte HMAC liée au Keystore lorsque le numéro est disponible. Le scanner manuel reste disponible sans permission. Sentinel contient désormais aussi un client SMS par défaut activable par l’utilisateur : composants `SMS_DELIVER`, `WAP_PUSH_DELIVER`, `SENDTO` et `RESPOND_VIA_MESSAGE`, envoi/réception SMS texte via Android et analyse smishing locale. Les permissions SMS restreintes ne sont demandées qu’après obtention réelle du rôle `ROLE_SMS` et l’accès cesse dès que Sentinel n’est plus gestionnaire par défaut. La restitution complète des MMS, les essais double SIM et la validation Play restent des gates de publication.
