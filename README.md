...
Le client SMS par défaut est désormais en phase de fondation : le manifeste déclare `SEND_SMS`, `RECEIVE_SMS` et `READ_SMS`, mais leur usage reste verrouillé derrière `ROLE_SMS` et la politique fail-closed `SmsRoleMigrationPolicy`. Le dépôt contient l’envoi via `SmsManager`, la réception `SMS_DELIVER`, l’écriture dans le provider SMS Android, le suivi envoyé/livré, `ACTION_SENDTO`, `RESPOND_VIA_MESSAGE` et l’analyse locale anti-fraude. Sentinel ne demande toujours pas le rôle SMS par défaut tant que MMS/WAP_PUSH, l’UX de conversations, le multi-SIM et la validation sur appareils physiques ne sont pas terminés.
