...
RENDERER="${SCRIPT_DIR}/render_unbound_blocklist.py"
...
[[ -f "${RENDERER}" ]] || fail "Missing render_unbound_blocklist.py next to installer."
...
install -d -m 0755 /usr/local/lib/sentinel-vpn
install -m 0755 "${RENDERER}" /usr/local/lib/sentinel-vpn/render_unbound_blocklist.py
python3 /usr/local/lib/sentinel-vpn/render_unbound_blocklist.py \
  --blocklist "${SENTINEL_BLOCKLIST_FILE}" \
  --allowlist "${SENTINEL_ALLOWLIST_FILE}" \
  --output "${BLOCK_CONF}"
