...
for value in   "${SENTINEL_WG_IPV4_ADDRESS}"   "${SENTINEL_WG_IPV6_ADDRESS}"   "${SENTINEL_WG_IPV4_SUBNET}"   "${SENTINEL_WG_IPV6_SUBNET}"   "${SENTINEL_ADMIN_SSH_CIDR}"   "${SENTINEL_EGRESS_INTERFACE}"; do
  [[ "${value}" != *
...
python3 -   "${SENTINEL_WG_IPV4_ADDRESS}"   "${SENTINEL_WG_IPV6_ADDRESS}"   "${SENTINEL_WG_IPV4_SUBNET}"   "${SENTINEL_WG_IPV6_SUBNET}"   "${SENTINEL_ADMIN_SSH_CIDR}"   "${SENTINEL_IPV6_EGRESS_READY}" <<'PY'
import ipaddress
import sys
...
# Require routing sanity before touching the firewall.
...
echo "DO NOT mark this gateway AVAILABLE until external IPv4/IPv6, DNS-leak, geolocation and reconnect tests pass."
\r'* ]] ||
    fail "Network values must not contain line breaks."
done
