...
**Mandatory isolation rule:** Sentinel must not import, bundle, deploy, authenticate against, or depend on another application or project. External project identifiers, package names, configuration, API credentials, assets and deployment configuration are prohibited.
...
HTTPS is mandatory for remote communications. Cleartext HTTP must be disabled unless a narrowly scoped exception is explicitly documented and justified. Remote endpoints must be reviewed and Sentinel must not call endpoints belonging to unrelated projects.
...
- [ ] No forbidden external project identifiers or artifacts
